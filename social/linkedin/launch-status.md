# LinkedIn - Nexo Digital Partners

## Pagina de servicios publicada: 2026-09-21

- URL: https://www.linkedin.com/services/page/68256b347374454192
- Servicios guardados y visibles: Desarrollo web, Diseño web, Desarrollo de software personalizado, Diseño de experiencia de usuario (UX).
- Modalidad En remoto y Consultar precios verificadas en la ficha. No se publico tarifa por hora ni se activo Premium.
- LinkedIn informa que los mensajes permanecen habilitados mientras los servicios esten publicados. La pagina fue creada para recibir solicitudes; no se enviaron invitaciones ni mensajes a terceros.
- Descripcion de 440 caracteres publicada, sin clientes, años de experiencia ni resultados inventados:

Diseñamos y desarrollamos webs profesionales, landing pages, formularios, dashboards y microaplicaciones para profesionales y pequeños negocios. Trabajamos a distancia y definimos alcance, entregables y presupuesto antes de comenzar. Nuestro portafolio incluye proyectos propios y prototipos, identificados como tales. También preparamos experiencias multilingües y flujos de trabajo asistidos por IA según las necesidades de cada proyecto.

- Dos muestras enlazadas añadidas a Contenido multimedia y verificadas tras recargar la ficha: Nexo | Web profesional multilingüe propia (https://josuest-b.github.io/nexo-digital-partners/) y Nexo Brief | Herramienta propia ES / EN (https://josuest-b.github.io/nexo-digital-partners/products/brief-kit/). No se usaron fotos personales ni se crearon publicaciones adicionales en el feed.
- La primera muestra identifica el sitio como proyecto propio, responsive y con 12 idiomas, sin atribuir resultados comerciales. La segunda describe objetivos y alcance, guardado local, exportaciones JSON/Markdown, privacidad y limites: sin consultoria ni generacion automatica con IA. LinkedIn confirmo Contenido multimedia añadido para la primera y ambas persistieron en la ficha despues de Guardar/Hecho y recarga.

## Perfil de empresa

Verificado el 11 de septiembre de 2026.

- Pagina: https://www.linkedin.com/company/nexo-digital-partners-ec/
- Logotipo cargado y guardado.
- Portada `assets/brand/linkedin-banner-nexo.png` guardada; confirmacion de LinkedIn y revision como miembro.
- Doce especialidades guardadas: desarrollo web, landing pages, automatizacion de procesos, IA aplicada, dashboards, microaplicaciones, catalogos, experiencia de usuario, webs multilingues, prototipos, formularios y arquitectura de contenido.
- Descripcion en espanol conservada y version inglesa agregada. Espanol sigue como idioma principal. No se han agregado las otras diez traducciones de la web a LinkedIn.
- No se ha usado el rostro del fundador en los activos de Nexo.
- No se han comprado anuncios, contratado Premium ni enviado mensajes a contactos.

## Primera publicacion

Confirmacion de LinkedIn: "Se ha publicado". Texto e imagen revisados en la publicacion resultante.

https://www.linkedin.com/feed/update/urn:li:share:7504194026607378432/

Imagen: `social/content/media/01-nexo-brand.png`.

Texto alternativo: Identidad de Nexo Digital Partners: monograma N metalico en plata y turquesa sobre fondo negro. Texto: Nexo Digital Partners. Sistemas, IA, productos web.

### Texto publicado

Presentamos Nexo Digital Partners.

Construimos herramientas digitales para profesionales y pequenos negocios que necesitan presentar sus servicios, recibir consultas y organizar mejor su trabajo.

Nuestro trabajo reune:
- Webs profesionales y paginas de venta.
- Formularios, catalogos y experiencias interactivas.
- Dashboards, microaplicaciones y automatizaciones.
- Arquitectura de contenido y experiencias multilingues.

No todos los proyectos necesitan un sistema grande. Empezamos por el problema concreto, definimos el alcance y construimos una primera solucion que se pueda probar.

En el portafolio compartimos proyectos propios y prototipos en salud, educacion, servicios e inventario.

Mira ejemplos y cuentanos que necesitas resolver:
https://josuest-b.github.io/nexo-digital-partners/

#DesarrolloWeb #Automatizacion #NexoDigitalPartners

Nota: esta transcripcion ASCII conserva el contenido; la publicacion original incluye tildes y vinetas.

## Continuidad

Evitar duplicar la presentacion. Las siguientes piezas son casos concretos, con capturas reales y sin atribuir ventas, clientes ni resultados que no esten documentados.

Actualizacion 2026-09-15: Nexo Brief anunciado como recurso gratuito con aportacion opcional, desde la pagina de Nexo y con visibilidad global. Texto y enlace de Gumroad, sin subir fotos nuevas. Confirmacion y publicacion resultante verificadas: https://www.linkedin.com/feed/update/urn:li:share:7505708559339384833/ . Transcripcion y limites en `brief-post-2026-09-15.md`.
